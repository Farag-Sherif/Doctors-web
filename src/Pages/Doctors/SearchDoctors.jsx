...
const SearchDoctors = () => {
    ...
    console.log(location.pathname.split('/').pop().replaceAll('%20', ' ').split('&'));
    const filterChosen = {
        specialties: location.pathname.split('/').pop().replaceAll('%20', ' ').split('&')[0] || "",
        city: location.pathname.split('/').pop().replaceAll('%20', ' ').split('&')[1] || "",
        country: location.pathname.split('/').pop().replaceAll('%20', ' ').split('&')[2] || "",
        doctorName: location.pathname.split('/').pop().replaceAll('%20', ' ').split('&')[3] || "",
    }
    console.log(filterChosen)
    const filteredDoctors = doctors.filter((doctor) => {
        // If no filter is chosen, return all doctors
        if (!filterChosen || (!filterChosen.specialties && !filterChosen.city && !filterChosen.country)) {
            return true;
        }
            ...
            <div className="doctors grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-5">
                {filteredDoctors.map((doctor) => (
                    <ElementBox
                        ...
                        specialty={doctor.specialties.map(s => s.toLowerCase()).join(", ")}
